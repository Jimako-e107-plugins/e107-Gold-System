...
if (e_PAGE == 'request.php' && $gold_obj->plugin_active('gold_download') && strpos(e_QUERY,'download.')===false)
{
    ...
    if ($_SESSION['gold_dl'] === 0 || e_QUERY != $gold_tmp[1])
    {
        ...
        header('Location:' . SITEURL . $PLUGINS_DIRECTORY . 'gold_download/gold_dl.php?' . e_QUERY);
        exit;
    }
    else
    {
        ...
        $gold_sql->db_Select('download', 'download_id,download_name', 'where download_id="' . $gold_request . '" or download_name="' . $gold_request . '"', 'nowhere', true);
