...
if($gasset_sql->db_Select_gen($gasset_arg,false))
{
	while($gasset_row=$gasset_sql->db_Fetch())
	{
		...
		if(!empty($gasset_asset) && is_readable(e_PLUGIN.'gold_asset/assets/'.$gasset_asset.'/asset.php'))
		{
			...
			$retval.='<span class="tooltip_text" onmouseover="showToolTip(event,\''.$gasset_title.'\');return false" onmouseout="hideToolTip()"><img src="'.$gasset_imagesrc.'" style="border:0px" alt="'.$gasset[$gasset_asset]['title'].'" title="'.$gasset[$gasset_asset]['title'].'" />&nbsp;</span>';
		}
		else
		{
			return '';
